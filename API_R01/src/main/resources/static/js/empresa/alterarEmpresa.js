...
document.addEventListener('DOMContentLoaded', function () {    
    ...
    .then(empresa => {
        ...
        img_foto_preview.setAttribute('src','data:image/jpeg;base64,'+empresa.foto);
        
        FOTO_ATUAL = empresa.foto;
...
async function alterarEmpresa(){
    ...
    .then(response => {
        if(response.ok) {
            document.body.style.cursor = "default"
            btn_submit.style.opacity = "1";
            notificar('Empresa alterada com sucesso!','sucesso');
            setTimeout( () => {
                location.reload();
            },5000);
        } else {
            notificar('Erro ao alterar Empresa','erro');
            document.body.style.cursor = "default"
            btn_submit.style.opacity = "1";
        }
    })
